...
export default function Home(){
 const [phase,setPhase]=useState<"intro"|"boot"|"game"|"recap"|"ending">("intro"),[act,setAct]=useState(0),[recapAct,setRecapAct]=useState(0),[recapArchive,setRecapArchive]=useState(false),[open,setOpen]=useState<number|null>(null),[answers,setAnswers]=useState(["","",""]),[solved,setSolved]=useState([false,false,false]),[final,setFinal]=useState(""),[finalOk,setFinalOk]=useState(false),[error,setError]=useState(""),[choices,setChoices]=useState<Choice[]>([]),[ending,setEnding]=useState("");
 ...
 const continueAct=()=>{setAct(recapAct+1);setAnswers(["","",""]);setSolved([false,false,false]);setFinal("");setFinalOk(false);setError("");setPhase("game")};
